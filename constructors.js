...
Movie.prototype.changeRating = function(newRating)  {
  let average = (this.rating + newRating) / 2; 
  this.rating = average; 
  return this.rating; 
}
...
// Write a prototype method for the User constructor function named removeSavedPost that will take in one number parameter representing the 
//post id. Use this id to find and remove the matching object in the savedPosts array.

User.prototype.removeSavedPost = function(num)
{
  let index = this.savedPosts.findIndex( post => post.id === num); 
  if (index !== -1) this.savedPosts.splice(index, 1); 
}
...
//The first will be an id (a number) and the second will be the new rating (a number). Use the id to find the matching object in the 
//savedPosts array. Once you find the matching object, update it's rating score with the new rating parameter.

User.prototype.changePostRating = function(id, newR)
{
  let index = this.savedPosts.findIndex (post => post.id === id); 
  if(index !== -1) this.savedPosts[index].rating = newR
}
